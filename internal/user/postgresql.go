...
type Storage interface {
	...
	//GetAllUsers(ctx context.Context) ([]GetUsersDto, error)
	GetOneUserById(ctx context.Context, id string) (u GetUsersDto, err error)
	GetOneUserByEmail(ctx context.Context, email string) (u GetUsersDto, err error)
	//UpdateUser(ctx context.Context, id string, user *UpdateUserDto) error
	//DeleteUserById(ctx context.Context, id string) error
	//CreateRating(ctx context.Context, dto RateDto, userId, ratedBy string) error
	...
	//GetAllUserRatings(ctx context.Context, id string) ([]GetAllRatingsDto, error)
	//CreateApplication(ctx context.Context, id string, filename string) error
	//GetApplications(ctx context.Context) ([]ApplicationDto, error)
...
func NewRepository(client *pgxpool.Pool) Storage {
	return &Repository{
		client: client,
	}
}

func (r *Repository) CreateUser(ctx context.Context, user *CreateUserDto) error {
	userQuery := `
	...
	exec, err := r.client.Exec(ctx, userQuery, id, user.Email, user.Password)
	if err != nil {
	...
	log.Println("execution result", exec.RowsAffected())
...
//
//func (r *Repository) GetAllUsers(ctx context.Context) ([]GetUsersDto, error) {
//	query := `
...
func (r *Repository) GetOneUserById(ctx context.Context, id string) (u GetUsersDto, err error) {
	query := `
	...
	log.Printf("SQL query: %s", postgresql.FormatQuery(query))
	err = r.client.QueryRow(ctx, query, id).Scan(&u.Id, &u.Email, &u.Password, &u.Role)
	if err != nil {
		log.Println(err)
		return u, err
	}
...
func (r *Repository) GetOneUserByEmail(ctx context.Context, email string) (u GetUsersDto, err error) {
	query := `
		SELECT id, email, password, role
		FROM public.users
		WHERE email = $1
	...
	log.Printf("SQL query: %s", postgresql.FormatQuery(query))
	err = r.client.QueryRow(ctx, query, email).Scan(&u.Id, &u.Email, &u.Password, &u.Role)
	if err != nil {
...
//func (r *Repository) UpdateUser(ctx context.Context, id string, user *UpdateUserDto) error {
//	query := `
//		UPDATE public.users
//		SET email = $1,
//		    password = $2,
...
//}
//
//func (r *Repository) CreateRating(ctx context.Context, dto RateDto, userId, ratedBy string) error {
//	query := `
...
//}
//
//func (r *Repository) GetAllUserRatings(ctx context.Context, id string) ([]GetAllRatingsDto, error) {
//	query := `
...
//	}
//
//	var ratings []GetAllRatingsDto
//	for rows.Next() {
//		var rate GetAllRatingsDto
//
...
//}
//
//func (r *Repository) GetApplications(ctx context.Context) ([]ApplicationDto, error) {
//	query := `
